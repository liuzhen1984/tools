...
import com.hillstone.hsa.index.utils.QueryResult;
...
public class LogSearch {
    ...
    public LogSearch(String logDate){
        this.logDate = logDate;
        new Date().getTime();
    }

    public LogSearch(String logDate,Integer startTime,Integer endTime){
        this.logDate = logDate;
        setStartTime(startTime);
        setEndTime(endTime);
    }

    public QueryResult searchFuzzSearch(String queryString, int firstResult, int max){
        Query query = new FuzzyQuery(new Term("log", queryString));
        System.out.println(query.toString());
        return search(query,firstResult,max);
    }
    /**
     * * 搜索
     *
...
//        }
        ...
        if(0!=getStartTime() && 0!=getEndTime()){
            Query query = NumericRangeQuery.newIntRange("receive_time",Integer.valueOf(getStartTime()),Integer.valueOf(getEndTime()),true,true);
            booleanQuery.add(query,BooleanClause.Occur.MUST);
        }
    ...
    private QueryResult search(Query query,Integer firstResult,Integer maxResults){
        ...
        String path = Configuration.PATH+"/"+this.logDate;
        try {
